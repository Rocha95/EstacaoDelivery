...
## Regressão — Correções V18

| ID | Cenário | Resultado esperado |
|---|---|---|
| PED-REG-001 | Cliente escolhe **Delivery** e confirma pedido | `Pedido.tipoEntrega = DELIVERY` no banco e no Painel |
| PED-REG-002 | Cliente escolhe **Retirada no local** | `Pedido.tipoEntrega = RETIRADA` no banco e no Painel |
| PED-REG-003 | Cliente envia `delivery` em minúsculo para a API | Backend normaliza para `DELIVERY` |
| PED-REG-004 | Tela Pedidos do Painel é aberta com banco contendo pedidos | Tela renderiza sem erro e lista os pedidos |
| PED-REG-005 | Pedido Delivery aparece no Painel | Coluna de recebimento mostra `Delivery` |
| PED-REG-006 | Pedido Retirada aparece no Painel | Coluna de recebimento mostra `Retirada no local` |
| PED-REG-007 | Tela de Estabelecimentos | Termos são orientados ao usuário: loja, unidade, loja atual, administrar |
| PED-REG-008 | Avançar status do pedido | API usa a rota `/api/pedidos/:id/status` e retorna erro legível em caso de falha |


## Agendamento de pedidos

- **PED-AGD-001** — Pedido agendado para o futuro não pode entrar em produção antes do horário.
- **PED-AGD-002** — Painel deve desabilitar a ação de iniciar produção enquanto o pedido estiver agendado para o futuro.
- **PED-AGD-003** — Ao tentar iniciar um pedido agendado, apresentar mensagem amigável com data/hora e orientação.
- **PED-AGD-004** — KDS deve aplicar a mesma regra de bloqueio de agendamento.
- **PED-AGD-005** — Pedido pode entrar em produção a partir do horário agendado.
- **PED-AGD-006** — Pedido sem agendamento continua podendo seguir o fluxo normalmente.
